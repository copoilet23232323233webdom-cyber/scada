...
class ModbusServiceV2:
    ...
    def __init__(self):
        ...
        self._executor = ThreadPoolExecutor(max_workers=12, thread_name_prefix='modbus')
    ...
    async def discover_cards(self, ip: str, id_start: int, id_end: int, 
        ...
        """
        Descubre tarjetas activas en un Gateway
        Escanea todos los IDs y retorna los que respondan
        """
        active_cards = []
        
        # Sondeo concurrente (limitado) en lugar de secuencial 32×timeout.
        # Así descubrir las tarjetas activas tarda ~2-3s en vez de ~64s.
        sem = asyncio.Semaphore(10)

        async def probe(card_id: int):
            async with sem:
                try:
                    regs = await self.modbus_read_async(ip, card_id, self.REG_STATUS, 1, min(timeout, 1.5))
                    if regs:
                        return card_id
                except:
                    pass
        ...
        results = await asyncio.gather(*[probe(cid) for cid in range(id_start, id_end + 1)])
        active_cards = [cid for cid in results if cid is not None]
    ...
    async def scan_gateway_multi(self, ip: str, id_start: int, id_end: int, 
                                timeout: float = None) -> GatewayFinalResult:
        """
        Realiza 5 escaneos de un Gateway y consolida resultados
        Solo genera alarmas si los 5 escaneos muestran el mismo error
        """
        if timeout is None:
            timeout = self.TIMEOUT_MS / 1000.0
        ...
        # Realizar 5 escaneos
        for scan_num in range(self.SCAN_COUNT):
            logger.info(f"Gateway {ip} - Escaneo {scan_num + 1}/{self.SCAN_COUNT}")
            scan_start = time.time()
            
            # Descubrir tarjetas activas
            active_cards = await self.discover_cards(ip, id_start, id_end, timeout)
            
            # Escanear cada tarjeta
            cards_data = []
            for card_id in active_cards:
                card_result = await self.scan_card(ip, card_id, timeout)
                cards_data.append({
                    "modbus_id": card_id,
                    "success": card_result.success,
                    "communication_ok": card_result.communication_ok,
                    "lora_ok": card_result.lora_ok,
                    "sec_alarm": card_result.sec_alarm,
                    "overvoltage_alarm": card_result.overvoltage_alarm,
                    "voltage": card_result.voltage,
                    "response_time_ms": card_result.response_time_ms,
                    "error": card_result.error_message
                })
            ...
            scan_results.append({
                ...
                "cards": cards_data,
                "duration_ms": (time.time() - scan_start) * 1000,
                "success": len(cards_data) > 0
            })
            
            if scan_num < self.SCAN_COUNT - 1:
                await asyncio.sleep(1)  # Esperar entre escaneos
        
        # Consolidar resultados de 5 escaneos
        final_result = self._consolidate_scans(ip, scan_results)
        final_result.response_time_ms = (time.time() - start_time) * 1000
        final_result.per_scan_time_ms = sum(sr.get('duration_ms', 0) for sr in scan_results) / len(scan_results) if scan_results else 0
    ...
    async def scan_gateway(self, ip: str, id_start: int = 1, id_end: int = 32) -> Dict:
        """
        Interfaz compatible con versión anterior
        Realiza scan con validación de 5 escaneos
        """
